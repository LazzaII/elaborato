<?php
    ...
    //$expID = explode(',', $_GET['badge_id']); //per fixare la url strana che da tasker
    //$badge_id =  $expID[0]; //Prende il numero di badge dal get 


    $badge_id = $_GET['badge_id']; //test da locale
    try {
        ...
        if(empty($check_badge)) header('Location: ../pages/access_refused.html'); 
            ...
            $check_exit = $db->query('SELECT gym_access.id
                                      ...
                                      WHERE gym_access.exit_day = "0000-00-00" AND gym_access.exit_hour = "00:00:00" AND teacher.badge_id = :badge_id', 'S',
                                      ['badge_id' => $badge_id]); 
            ...
            if(!empty($check_exit)) {
                ...
                $day = date("Y-m-d");
                $hour = date("H:i:s");
                $db->query('INSERT INTO gym_access (teacher, access_day, access_hour, exit_day, exit_hour) 
                            VALUES (:t_id, :dday, :hour, "0000-00-00", "00:00:00")', 'I',
                            [
